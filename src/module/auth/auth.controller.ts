...
export class AuthController {
    ...
    async signin(@Body() model: SignInDto) {
        try{
            let user = await this.auth.validateUser(model.username, model.password);
            if(user){
                let token = this.auth.signJwt(user);
                return {...user, token}
            }else {
                return new NotFoundException(new ErrorResponse(400, "Không tìm thấy tài khoản trùng khớp", "MSE001"))
            }
        }catch(error){
            return new BadRequestException(
                new ErrorResponse(500, error.message, "MSE00")
